    ...
    if df.empty:
        ...
        with r1c1:
            ...
            fig = go.Figure(data=[go.Pie(
                ...
                hole=0.55,
                marker=dict(colors=[color_map.get(x, "#94a3b8") for x in risk_counts["Risk Level"]]),
                textposition="inside",
        ...
        with r2c1:
            ...
            fig3 = go.Figure(data=[go.Bar(
                ...
                marker=dict(
                    color=[{"Local": "#60a5fa", "Cloud": "#ffa94d"}[s] for s in source_counts["Source"]],
                    line=dict(color="#1e2535", width=1)
                ),
        ...
        with r2c2:
            ...
            for risk in ["Critical", "Medium", "Low"]:
                ...
                fig4.add_trace(go.Bar(
                    ...
                    y=risk_data["count"],
                    name=risk,
                    marker=dict(color=color_map[risk], line=dict(color="#1e2535", width=0.5)),
                    hovertemplate="<b>%{x}</b> - " + risk + "<br>%{y} findings<extra></extra>"
                ))
        ...
        if len(display) > 0:
            st.dataframe(
                ...
                column_config={
                    ...
                    "card_number": st.column_config.TextColumn("Card (Masked)", width="small"),
                    "risk_level": st.column_config.TextColumn("Risk", width="small"),
                    "scan_date": st.column_config.TextColumn("Scan Date", width="medium"),
                },
            )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with sc2:
        use_cloud = st.checkbox("☁️ Google Drive (Cloud)", value=False)
    with sc3:
    ...
    if use_cloud and not use_sample:
        ...
        with gdrive_col2:
            ...
            if not os.path.exists(creds_file):
                st.warning(
                    "⚠️ credentials.json not found — Google Drive scan will be skipped"
                )
    ...
    if st.session_state.scan_log or scan_running:
        ...
        for line in log_lines[-80:]:
            if any(
                ...
                colored.append(f'<span class="log-line-info">ℹ {line}</span>')
            else:
                colored.append(line)
    ...
    if use_local and not use_sample and (not scan_running):
        if paths and len(paths) > 1:
            ...
            <div style="background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.25);
                ...
                <span style="color:#60a5fa;font-weight:700">ℹ️ Multi-path mode:</span>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df.empty:
        ...
        with fc1:
            risk_opts = sorted(df["risk_level"].dropna().unique().tolist())
            risk_filter = st.multiselect("Risk Level", risk_opts, default=risk_opts, key="ai_risk")
        with fc2:
            source_filter = st.multiselect(
                "Source", ["Local", "Cloud"], default=["Local", "Cloud"], key="ai_source"
            )
        with fc3:
            search_ai = st.text_input("Search file", placeholder="filter by path...", key="ai_search")
        ...
        if len(filtered_df) > 0:
            for idx, (_, row) in enumerate(filtered_df.iterrows()):
                ...
                with st.expander(f"📄 {row['file'][:60]}... {badge}", expanded=(idx==0)):
                            ...
                            try:
                                ...
                                if not success:
                                    ...
                                    if "Google Drive" in message:
                                        st.success("✓ Remediated locally and uploaded to Google Drive!")
                                    else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if remediated_df.empty:
        st.info("✓ No remediated findings yet. Remediate findings from the AI Analysis tab.")
    else:
        st.markdown(
        ...
        with fc2:
            risk_opts_rem = sorted(remediated_df["risk_level"].dropna().unique().tolist())
        ...
        with fc3:
            if st.button("🧹 Clear Remediated Data", use_container_width=True, type="secondary"):
                try:
        ...
        filtered_rem = remediated_df[remediated_df["risk_level"].isin(selected_risks_rem)]
        if search_rem:
            filtered_rem = filtered_rem[
                (filtered_rem["original_file"].str.contains(search_rem, case=False, na=False)) |
                (filtered_rem["remediated_file"].str.contains(search_rem, case=False, na=False))
            ]
                ...
                with st.expander(f"✓ {display_name[:60]}... {badge}"):
                    ...
                    with left:
                        ...
                        if row.get('original_file'):
                            st.markdown(f"**Original:** `{row['original_file'][:50]}...`")
                        if row.get('remediated_file'):
                            st.markdown(f"**Remediated:** `{row['remediated_file'][:50]}...`")
                        ...
                        if st.button(
                            ...
                            try:
                                ...
                                cursor = conn.cursor()
                                cursor.execute(
                                    "DELETE FROM remediated_findings WHERE id = ?", (row["id"],)
                                )
                                conn.commit()
                                conn.close()
                                load_remediated_data.clear()
