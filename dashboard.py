...
for i, (col, tab_name) in enumerate(zip([col1, col2, col3], tabs)):
    with col:
        ...
        if st.button(
            tab_name, use_container_width=True, key=f"tab_{i}", type=button_type
        ):
...
if df.empty:
    if st.session_state.active_tab == 0:
        st.warning(
            "No findings database found. Please run `python main.py` and `python create_db.py` first."
        )
    elif st.session_state.active_tab == 1:
        st.info("No data available. Please run a scan first to see detailed analysis.")
    elif st.session_state.active_tab == 2:
        st.info(
            "No report available. Please run a scan first to generate the executive report."
        )
else:
    # Top Level Metrics
    ...
    if "remediation" in df.columns:
        remediated_count = len(
            df[df["remediation"].str.contains("Masked and saved", na=False)]
        )
    else:
        remediated_count = 0
    ...
    if st.session_state.active_tab == 0:
        ...
        with col3:
            ...
            df["source"] = df["file"].apply(
                lambda x: "Cloud Files" if x.startswith("gdrive://") else "Local Files"
            )
            ...
            fig3 = px.bar(
                ...
                y="Count",
                color="Source",
                color_discrete_map={"Local Files": "#1f77b4", "Cloud Files": "#ff7f0e"},
            )
        ...
        with col4:
            ...
            def categorize_file_type(file_path):
                ...
                if "config" in file_lower or file_lower.endswith(
                    (".env", ".ini", ".conf")
                ):
            ...
            file_type_risk = (
                df.groupby(["file_type", "risk_level"]).size().reset_index(name="count")
            )
            ...
            fig4.update_layout(
                xaxis_title="File Type", yaxis_title="Number of Findings"
            )
        ...
        if search_file:
            filtered_df = filtered_df[
                filtered_df["file"].str.contains(search_file, case=False)
            ]
        ...
        if "remediation" in filtered_df.columns:
            display_df["remediation_status"] = filtered_df["remediation"].apply(
                lambda x: (
                    "Remediated" if "Masked and saved" in str(x) else "Not remediated"
                )
            )
        display_df["card_number"] = display_df["card_number"].apply(
            lambda x: f"{x[:4]}****{x[-4:]}"
        )
        ...
        st.markdown(
            "Deep dive into the LLM's reasoning for risk and security conditions."
        )
        ...
        for index, row in filtered_df.iterrows():
            with st.expander(f"{row['file']} | Risk: {row['risk_level']}"):
                ...
                with col1:
                    st.markdown(
                        f"**Card Number:** {row['card_number'][:4]}****{row['card_number'][-4:]}"
                    )
                    st.markdown(f"**Risk Level:** {row['risk_level']}")
                    st.markdown(
                        f"**Source:** {'Cloud' if row['file'].startswith('gdrive://') else 'Local'}"
                    )
                ...
                with col2:
                    cleaned_analysis = clean_context_analysis(
                    ...
                    if (
                        "remediation" in row
                        and row["remediation"]
                        and "Masked and saved" in str(row["remediation"])
                    ):
        ...
        try:
            ...
            st.error(
                "Report could not be loaded. Please run a scan first to generate the executive report."
            )
