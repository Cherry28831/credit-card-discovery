...
def context_agent(state):
    ...
    print(f"    Processing {total} findings in {num_batches} batches of {batch_size}", flush=True)
    
    for batch_num in range(num_batches):
        batch_start = batch_num * batch_size
        batch_end = min(batch_start + batch_size, total)
        print(f"    Batch {batch_num + 1}/{num_batches} ({batch_end - batch_start} findings)...", flush=True)
...
Keep total response under 100 words. Do not use brackets."""
    ...
    print(f"  AI context analysis complete for {total} findings", flush=True)
    return state
